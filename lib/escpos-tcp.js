...
function renderEscPosDocument(document, codepage) {
    ...
    const styled = (line, style) => {
        ...
        else if (style === 'double-height') parts.push(STYLE.doubleHeight, encode(line), STYLE.normalSize);
    ...
    lines.forEach((line, index) => {
        ...
        const waiter = /^(?:(Mesa\s+.*?\s+\/\s+))?(Mozo\s+.*)$/.exec(line);

        if (title) styled(line, 'bold');
        else if (ticket) boldThenNormal(ticket[1], ticket[2]);
        else if (waiter) partialBold(waiter[1] || '', waiter[2]);
        else if (movement) {
            styled(line, 'normal');
            ...
            styled(line, 'double-height');
            detailStyle = 'note';
        } else if (detailStyle === 'product') styled(line, 'bold');
        else if (detailStyle === 'note') styled(line, 'double-height');
    ...
    parts.push(STYLE.boldOff, STYLE.normalSize);
