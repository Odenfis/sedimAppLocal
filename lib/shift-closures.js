...
function install(app, auth, broadcast) {
    const handler = fn => async (req, res) => { try { res.json(await fn(req)); } catch (error) { res.status(error.status || 500).json({ success: false, message: error.status ? error.message : 'No se pudo completar el cierre de turno.' }); } };
