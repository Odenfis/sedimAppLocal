...
async function preview(db, scope) {
    ...
    const blockers = (await request(db, `WITH Scope AS (
        ...
        (SELECT COUNT(DISTINCT jobs.NroTicket) FROM (
            SELECT e.NroTicket FROM Impresion_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId JOIN Scope s ON s.NroTicket=e.NroTicket WHERE j.Estado IN('en_cola','procesando')
            UNION ALL
            SELECT e.NroTicket FROM Impresion_barra_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId JOIN Scope s ON s.NroTicket=e.NroTicket WHERE j.Estado IN('en_cola','procesando')
        ) jobs) ImpresionesActivas`, values)).recordset[0] || {};
...
async function archiveTicket(db, cierreId, scope, nroTicket) {
    ...
    const barJobs = (await request(db, `SELECT j.* FROM Impresion_barra_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId
        WHERE e.NroTicket=@nro ORDER BY j.Fecha,j.Id`, { nro: nroTicket })).recordset;
    const routes = (await request(db, `SELECT r.* FROM Impresion_linea_rutas r JOIN Pedido_lineas l ON l.LineaId=r.LineaId
        WHERE l.NroTicket=@nro ORDER BY r.LineaId`, { nro: nroTicket })).recordset;
    const payload = JSON.stringify({ schemaVersion: 2, scope, nroTicket, tables: {
        ...
        Impresion_barra_trabajos: barJobs, Impresion_linea_rutas: routes
    }});
    ...
    return { hash, counts: { Pedido_control: control ? 1 : 0, Pedido_lineas: lines.length, Cocina_envios: sends.length,
        ...
        Impresion_barra_trabajos: barJobs.length, Impresion_linea_rutas: routes.length } };
}
...
async function purgeClosure(id, user, key = randomUUID(), automatic = false) {
    ...
    try { return await txRun(async db => {
        ...
        for (const archive of archives) {
            ...
            await request(db, `UPDATE j SET ReimpresionDe=NULL FROM Impresion_barra_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId WHERE e.NroTicket=@nro;
                DELETE j FROM Impresion_barra_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId WHERE e.NroTicket=@nro;
                UPDATE j SET ReimpresionDe=NULL FROM Impresion_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId WHERE e.NroTicket=@nro;
