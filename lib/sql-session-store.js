...
const { measureSql } = require('./request-context');
...
class SqlSessionStore extends session.Store {
    ...
    async get(sid, callback) {
        try {
            ...
            const request = pool.request().input('sid', sql.VarChar(128), sid);
            const row = (await measureSql(() => request.query('SELECT Data FROM Web_sessions WHERE Sid=@sid AND ExpiresAt>SYSUTCDATETIME()'))).recordset[0];
    ...
    async set(sid, value, callback = () => {}) {
        try {
            ...
            await measureSql(() => request.query(`MERGE Web_sessions WITH (HOLDLOCK) AS target
    ...
    async destroy(sid, callback = () => {}) {
        try {
            const pool = await getConnection();
            const request = pool.request().input('sid', sql.VarChar(128), sid);
            await measureSql(() => request.query('DELETE Web_sessions WHERE Sid=@sid'));
            callback();
        } catch (error) { callback(error); }
    }

    // The browser cookie is not rolling. Rewriting the full JSON session on every
    // authenticated GET only creates SQL write pressure without extending its use.
