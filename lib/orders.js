...
function install(app, auth, broadcast) {
    const route = (method, path, fn) => app[method](path, auth, async (req, res) => {
        try {
            ...
            if (!e.status) console.error(JSON.stringify({ type: 'api_error', diagnosticId, context: path, error: requestContext.safeError(e) }));
            res.status(e.status || 500).json({ success: false, message: e.status ? e.message : 'No se pudo completar la operación.', diagnosticId });
        }
    });
