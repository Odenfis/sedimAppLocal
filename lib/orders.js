...
async function createSend(db, t, clave, usuario) {
    ...
    const batch = movements.map(m => {
        ...
        return { detailId, lineaId: m.lineaId, codPro: l.codPro, movement: JSON.stringify(m), operative: JSON.stringify(operative),
            sent: m.nueva ? JSON.stringify(m.nueva) : null,
            mode: l.estadoCocina == null ? 'insert' : l.estadoCocina >= 2 ? 'pending' : !m.nueva ? 'cancel' : 'replace' };
    });
    const printSql = printerEnabled()
        ? `INSERT Impresion_trabajos(Id,EnvioId,Clave,Documento,Usuario) VALUES(@job,@envioId,@printKey,@doc,@usuario);`
        : '';
    await query(db, `DECLARE @batch TABLE(DetailId UNIQUEIDENTIFIER,LineaId UNIQUEIDENTIFIER,CodPro VARCHAR(10),Movement NVARCHAR(MAX),Operative NVARCHAR(MAX),Sent NVARCHAR(MAX),Mode VARCHAR(10));
        INSERT @batch SELECT DetailId,LineaId,CodPro,Movement,Operative,Sent,Mode FROM OPENJSON(@batchJson)
        WITH(DetailId UNIQUEIDENTIFIER '$.detailId',LineaId UNIQUEIDENTIFIER '$.lineaId',CodPro VARCHAR(10) '$.codPro',Movement NVARCHAR(MAX) '$.movement',Operative NVARCHAR(MAX) '$.operative',Sent NVARCHAR(MAX) '$.sent',Mode VARCHAR(10) '$.mode');
        ...
        { batchJson: JSON.stringify(batch), envioId, nro: t.NroTicket, numero, clave, version: t.Version, head: JSON.stringify(head), doc,
            usuario, job: randomUUID(), printKey: randomUUID() });
