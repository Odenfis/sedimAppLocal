...
async function ticket(db, nro, empresa, editable = false, forUpdate = true) {
...
async function response(db, t) {
    ...
    const printingRows = (await query(db, `SELECT Id,Estado,Error,Destino FROM (
            SELECT j.Id,j.Estado,j.Error,'cocina' Destino,j.Fecha,ROW_NUMBER() OVER(PARTITION BY 'cocina' ORDER BY j.Fecha DESC,j.Id) rn
            FROM Impresion_trabajos j JOIN Cocina_envios e ON e.Id=j.EnvioId WHERE e.NroTicket=@nro
            UNION ALL
...
async function createSend(db, t, clave, usuario) {
    ...
    const routeRows = (await query(db, `SELECT input.LineaId,r.Destino,p.Clinea,p.Tipo
        ...
        LEFT JOIN Impresion_linea_rutas r ON r.LineaId=input.LineaId
    ...
    const routes = new Map(routeRows.map(r => [r.LineaId.toLowerCase(), r.Destino || (Number(r.Clinea) === 7 && Number(r.Tipo) === 3 ? 'barra' : 'cocina')]));
    ...
    const kitchenMovements = movements.filter(m => routes.get(m.lineaId) !== 'barra');
    ...
    await query(db, `DECLARE @batch TABLE(DetailId UNIQUEIDENTIFIER,LineaId UNIQUEIDENTIFIER,CodPro VARCHAR(10),Destino VARCHAR(16),Movement NVARCHAR(MAX),Operative NVARCHAR(MAX),Sent NVARCHAR(MAX),Mode VARCHAR(10));
        ...
        INSERT Impresion_linea_rutas(LineaId,Destino)
            SELECT b.LineaId,MIN(b.Destino) FROM @batch b LEFT JOIN Impresion_linea_rutas r ON r.LineaId=b.LineaId
...
function install(app, auth, broadcast) {
    const route = (method, path, fn) => app[method](path, auth, async (req, res) => {
        try {
            ...
            if (e.status) return res.status(e.status).json({ success: false, message: e.message, diagnosticId, retryable: false });
    ...
    route('get', '/api/pos/pedido', async (db, req) => {
        ...
        return response(db, await ticket(db, found.NroTicket.trim(), cfg.empresa, false, false));
    });
    route('get', '/api/pos/pedido/:nro/estados-cocina', async (db, req) => {
        const t = await ticket(db, req.params.nro, req.query.empresa, false, false);
    ...
    route('get', '/api/pos/pedido/:nro/envios', async (db, req) => {
        ...
        const envios = (await query(db, `SELECT e.Id,e.Numero,e.Fecha,e.Historico,e.Documento,
            (SELECT j.Id,j.Estado,j.Error,j.Intentos,j.ReimpresionDe,j.Fecha,j.Destino FROM (
                SELECT Id,Estado,Error,Intentos,ReimpresionDe,Fecha,'cocina' Destino FROM Impresion_trabajos WHERE EnvioId=e.Id
                UNION ALL
    ...
    route('post', '/api/pos/pedido/:nro/envios/:envio/reimprimir', async (db, req) => {
        ...
        if (!['cocina','barra'].includes(destination)) fail('Destino de impresión inválido');
        if (destination === 'cocina' ? !printerEnabled() : !barPrinterEnabled()) fail(`La impresión de ${destination} todavía no está habilitada.`, 503);
        ...
        const duplicate = (await query(db, `SELECT Id,EnvioId FROM ${queue} WHERE Clave=@clave`, { clave })).recordset[0];
        if (duplicate) { if (duplicate.EnvioId.toLowerCase() !== envio) fail('Clave usada en otro envío', 409); return { success: true, trabajoId: duplicate.Id.toLowerCase(), destino: destination }; }
        const jobs = (await query(db, `SELECT * FROM ${queue} WHERE EnvioId=@envio ORDER BY Fecha DESC`, { envio })).recordset;
        ...
        await query(db, `INSERT ${queue}(Id,EnvioId,ReimpresionDe,Clave,Documento,Usuario)
            ...
            { job, envio, original: jobs[0].Id, clave, doc: '*** REIMPRESIÓN ***\n' + jobs[0].Documento, usuario: req.session.user.usuario });
    ...
    route('get', '/api/cocina/pedidos', async (db, req) => {
        ...
        const rows = (await query(db, `WITH active_lines AS (
            ...
            bj.Id TrabajoBarraId,bj.Estado EstadoImpresionBarra,bj.Error ErrorImpresionBarra,
            DATEDIFF(MINUTE,c.FechaEnvio,SYSUTCDATETIME()) MinutosEspera,L.Descripcion Categoria,
            em.Nombre MozoNombre
            FROM active_lines c JOIN Ticket_c t ON t.NroTicket=c.NroTicket
            ...
            OUTER APPLY (SELECT TOP 1 j.Id,j.Estado,j.Error FROM Impresion_barra_trabajos j
                WHERE j.EnvioId=e.Id ORDER BY j.Fecha DESC,j.Id) bj
        ...
        for (const r of rows) {
            const codPro = String(r.Codpro || '').trim();
            const parsedOperative = safeJson(r.Operativa, null, 'Cocina_estados.Operativa');
            if (!r.LineaId || !codPro) {
                console.warn(JSON.stringify({ type: 'invalid_kitchen_line', requestId: requestContext.currentId(),
                    ticket: String(r.NroTicket || '').trim(), lineId: r.LineaId?.toLowerCase?.() || null }));
                continue;
            }
            ...
            if (!grouped.has(ticket)) grouped.set(ticket, { nroTicket: ticket, envioId: r.EnvioId?.toLowerCase() || null,
                ...
                impresion: r.TrabajoId ? { id: r.TrabajoId.toLowerCase(), estado: r.EstadoImpresion, error: r.ErrorImpresion, destino: 'cocina' } : null,
                impresiones: [r.TrabajoId ? { id: r.TrabajoId.toLowerCase(), estado: r.EstadoImpresion, error: r.ErrorImpresion, destino: 'cocina' } : null,
                    r.TrabajoBarraId ? { id: r.TrabajoBarraId.toLowerCase(), estado: r.EstadoImpresionBarra, error: r.ErrorImpresionBarra, destino: 'barra' } : null].filter(Boolean), lineas: [] });
            const parsedCorrection = safeJson(r.Movimiento, null, 'Cocina_envio_detalles.Movimiento');
            const correction = r.PendienteId && !parsedCorrection
                ? { tipo: 'CORRECCIÓN PENDIENTE', datosIncompletos: true }
                : parsedCorrection;
            grouped.get(ticket).lineas.push({ lineaId: r.LineaId.toLowerCase(), codPro, cantidad: operative.cantidad,
                nombre: operative.nombre || codPro, notasRapidas: Array.isArray(operative.notasRapidas) ? operative.notasRapidas : [],
                nota: typeof operative.nota === 'string' ? operative.nota : '', estado: r.Estado,
                Categoria: r.Categoria, datosIncompletos: Boolean(operative.datosIncompletos), correccionPendiente: correction });
        }
        const pedidos = [...grouped.values()].filter(p => p.lineas.length).map(p => ({ ...p, estadoTicket: p.lineas.some(l => l.correccionPendiente) ? 'pendiente' : p.lineas.every(l => l.estado >= 3) ? 'listo' : p.lineas.every(l => l.estado >= 2) ? 'preparacion' : 'pendiente' }));
